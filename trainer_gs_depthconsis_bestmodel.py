...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth"] = networks.DepthDecoder_MSF_GS_FiT_FBT(
        ...
        if self.opt.use_gs and self.opt.gs_scale == 0:
            for scale in range(1, 6):
                if self.opt.v1_multiscale:
                    ...
                    source_scale = 0
                disp = outputs["output", 0][("disp", source_scale)]
                disp_gs = outputs["output", 0][("disp_init", 2**scale)]

                disp_gs = F.interpolate(
                        disp_gs, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
                loss_gs += self.compute_reprojection_loss(disp_gs, disp.detach()).mean()
