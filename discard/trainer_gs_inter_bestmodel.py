...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth"] = networks.DepthDecoder_MSF_GS_FiT_Inter(
        ...
        self.project_depth = {}
        self.detail_guide = {}
        self.backproject_depth_inter = {}
        self.project_3d_inter = {}
        ...
        for scale in range(1, 6):
            self.backproject_depth_inter[scale] = BackprojectDepth_removedxy(self.opt.batch_size, h // (2**scale), w // (2**scale))
            self.backproject_depth_inter[scale].to(self.device)
            self.project_3d_inter[scale] = Project3D_removedxy(self.opt.batch_size,  h // (2**scale), w // (2**scale))
            self.project_3d_inter[scale].to(self.device)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            if self.epoch < 15 and self.step % 1000 == 0:
                self.val(batch_idx)
            elif self.epoch >= 15 and self.step % self.opt.eval_frequency_best == 0:
                self.val(batch_idx)
            self.step += 1
    ...
    def process_batch(self, mode, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            if mode == 'val':
                ...
                outputs["output", 0] = self.models["depth"](features, inv_K, K, 0)
            else:
                if self.opt.use_interframe_consistency:
                    for f_i in self.opt.frame_ids:
                        if f_i == 0:
                            features = self.models["encoder"](inputs["color_aug", f_i, 0])
                            outputs["output", f_i] = self.models["depth"](features, inv_K, K, f_i)
                        else:
                            with torch.no_grad():  # 只推理，不计算梯度
                                features = self.models["encoder"](inputs["color", f_i, 0])
                                outputs["output", f_i] = self.models["depth"](features, inv_K, K, f_i)
                else:
        ...
        if self.opt.use_gs and self.opt.gs_scale == 0:
            ...
            if self.opt.use_interframe_consistency:
                for scale in range(1, 2):
                    for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                        gs_feature = outputs["output", frame_id][("gs_feature", 2**scale)]
                        gs_feature = F.interpolate(
                            gs_feature, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
                        outputs[("gs_feature", frame_id, scale)] = F.grid_sample(
                            gs_feature,
    ...
    def compute_losses(self, inputs, outputs):
        ...
        if self.opt.use_gs and self.opt.gs_scale == 0:
            ...
            if self.opt.use_interframe_consistency:
                for scale in range(1, 2):
                    reprojection_losses_gs_feature = []
                    target_gs_feature = outputs["output", 0][("gs_feature", 2**scale)]
                    target_gs_feature = F.interpolate(
                        target_gs_feature, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
                    for frame_id in self.opt.frame_ids[1:]:
                        pred_gs_feature = outputs[("gs_feature", frame_id, scale)]
                        reprojection_losses_gs_feature.append(torch.abs(target_gs_feature.detach() - pred_gs_feature).mean(1, True))

                    reprojection_losses_gs_feature = torch.cat(reprojection_losses_gs_feature, 1)

                    if self.opt.avg_reprojection:
                        reprojection_losses_gs_feature = reprojection_losses_gs_feature.mean(1, keepdim=True)
                    else:
                        reprojection_losses_gs_feature = reprojection_losses_gs_feature

                    to_optimise_gs_feature, _ = torch.min(reprojection_losses_gs_feature, dim=1)
                    loss_gs_inter += to_optimise_gs_feature.mean()
        ...
        if self.opt.use_gs and self.opt.gs_scale == 0:
            ...
            if self.opt.use_interframe_consistency:
                loss_gs_inter /= 1
                loss_gs_inter = self.opt.loss_interframe_weight * loss_gs_inter
                total_loss += loss_gs_inter
                losses["loss_gs_inter"] = loss_gs_inter
    ...
    def log_time(self, batch_idx, duration, losses):
        ...
        if self.opt.use_gs and self.opt.use_interframe_consistency:
            print_string = "epo {:>3} | bat {:>6} | ex/s: {:5.1f}" + \
                " | lM: {:.4f} | lM_gs: {:.6f} | lM_gs_inter: {:.6f} | loss: {:.5f} | te: {} | tl: {} | lr: {}"
            print(print_string.format(self.epoch, batch_idx, samples_per_sec, losses["loss"].cpu().data, losses["loss_gs"].cpu().data, losses["loss_gs_inter"].cpu().data, losses["total_loss"].cpu().data,
