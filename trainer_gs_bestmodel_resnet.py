...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth"] = networks.DepthDecoder_MSF_GS_FiTAlter(
        ...
        train_dataset = self.dataset(
            ...
            self.opt.frame_ids, self.num_scales, is_train=True, img_ext=img_ext, gs_scale=self.opt.gs_scale)
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, True,
            num_workers=self.opt.num_workers, pin_memory=True, drop_last=True)
        val_dataset = self.dataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width,
            self.opt.frame_ids, self.num_scales, is_val=True, img_ext=img_ext, gs_scale=self.opt.gs_scale)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            # ---------------------- 新增代码：监控梯度 ----------------------
            # if batch_idx % 10 == 0:  # 每隔10个batch监控一次
            #     total_grad_norm = 0.0
            #     for name, param in self.models():
            #         if param.grad is not None:
            #             grad_norm = torch.norm(param.grad, p=2)  # 计算L2范数
            #             grad_mean = param.grad.mean().item()
            #             grad_std = param.grad.std().item()
            #             total_grad_norm += grad_norm.item()
                        
            #             # 记录到日志（假设已有日志函数）
            #             print(f"grad/{name}_norm", grad_norm, self.step)
            #             print(f"grad/{name}_mean", grad_mean, self.step)
            #             print(f"grad/{name}_std", grad_std, self.step)
                
            #             # 记录总梯度范数
            #             print("grad/total_norm", total_grad_norm, self.step)
            # ----------------------------------------------------------------
    ...
    def process_batch(self, mode, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            if mode == 'val':
                ...
                outputs = self.models["depth"](features, inv_K, K)
            else:
    ...
    # def save_img(disp,target,scale,train_save_path,model_name):
    #     dn_to_disp_resized_np = disp[0].squeeze().cpu().detach().numpy()
    #     # 在保存前添加调试代码，打印数据统计信息
    #     # print(f"Scale {scale}: min={dn_to_disp_resized_np.min()}, max={dn_to_disp_resized_np.max()}, mean={dn_to_disp_resized_np.mean()}")
    #     # 检查各尺度视差图的数值范围
    #     print(f"Scale {scale}: min={disp.min().item()}, max={disp.max().item()}, mean={disp.mean().item()}")
    ...
    #     im.save(os.path.join(train_save_path, "{}_gs_disp_scale{}.png".format(model_name,scale)))
        
    #     target_ = target[0].cpu().detach().numpy() * 255
    #     target_ = np.transpose(target_, (1, 2, 0)).astype(np.uint8)
    #     target_ = Image.fromarray(target_)
    #     target_.save(os.path.join(train_save_path, "{}_image.png".format(model_name)))
