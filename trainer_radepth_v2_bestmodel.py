...
class Trainer:
    def __init__(self, options):
        ...
        self.models["depth"] = networks.DepthDecoder_MSF_RADepth_v1_GS_FiT_v2(
        ...
        # data
        datasets_dict = {"kitti": datasets.KITTIRAWDataset_RADepth_V1,
    ...
    def process_batch(self, mode, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            if mode == 'val':
                ...
                features_MiS = self.models["encoder"](inputs["color_MiS", 0, 0])
                outputs["out_MiS"] = self.models["depth"](features_MiS, inv_K_MiS, K_MiS, inputs[("dxy_MiS")])
            else:    
                inv_K_HiS = []
                K_HiS = []
                inv_K_MiS = []
                ...
                outputs["out_HiS"] = self.models["depth"](features_HiS, inv_K_HiS, K_HiS, inputs[("dxy_HiS")])
                # Otherwise, we only feed the image with frame_id 0 through the depth encoder
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i in range(self.opt.batch_size):
                if inputs[("do_HiS")][i] is True:
                    ...
                    width_un_LoS = int(inputs[("resize_HiS")][i,0])
                    height_un_LoS = int(inputs[("resize_HiS")][i,1])
                    disp_un_LoS = F.interpolate(disp_HiS[i,:,0:height_un_LoS,0:width_un_LoS].unsqueeze(0), [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
                    ...
                    disp_diff_batch += (disp_diff_LoS_mask.sum() / (mask_disp_LoS.sum() + 1e-7))
            loss_consis_HiS += disp_diff_batch / self.opt.batch_size

            if self.opt.use_gs and self.opt.gs_scale != 0:
                loss_gs_HiS += self.compute_reprojection_loss(disp_gs_HiS, disp_HiS.detach()).mean()
                loss_gs_MiS += self.compute_reprojection_loss(disp_gs_MiS, disp_MiS.detach()).mean()
        ...
        if self.opt.use_gs and self.opt.gs_scale == 0:
            for scale in range(1, 6):
                if self.opt.v1_multiscale:
                    ...
                    source_scale = 0
                disp_HiS = outputs["out_HiS"][("disp", source_scale)]
                disp_MiS = outputs["out_MiS"][("disp", source_scale)]
                disp_gs_HiS = outputs["out_HiS"][("disp_init", 2**scale)]
                disp_gs_MiS = outputs["out_MiS"][("disp_init", 2**scale)]
                disp_gs_HiS = F.interpolate(
                        disp_gs_HiS, [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
        ...
        total_loss = (loss_HiS + loss_MiS) + 1.0*(loss_consis_HiS)

        if self.opt.use_gs and self.opt.gs_scale == 0:
            loss_gs_HiS /= 5
            loss_gs_MiS /= 5
            loss_gs = self.opt.loss_gs_weight * (loss_gs_HiS + loss_gs_MiS)
            total_loss += loss_gs
    ...
    def log_time(self, batch_idx, duration, losses):
        ...
        if self.opt.use_gs:
            print_string = "epo {:>3} | bat {:>6} | ex/s: {:5.1f}" + \
                " | lH: {:.4f} | lM: {:.4f} | cH: {:.10f} | lM_gs: {:.4f} | loss: {:.5f} | te: {} | tl: {} | lr: {}"
            print(print_string.format(self.epoch, batch_idx, samples_per_sec, losses["loss_HiS"].cpu().data, losses["loss_MiS"].cpu().data, 
                                    losses["loss_consis_HiS"].cpu().data, losses["loss_gs"].cpu().data, losses["loss"].cpu().data,
                                    sec_to_hm_str(time_sofar), sec_to_hm_str(training_time_left), self.model_optimizer.state_dict()['param_groups'][0]['lr']))
        else:
            print_string = "epo {:>3} | bat {:>6} | ex/s: {:5.1f}" + \
                " | lH: {:.4f} | lM: {:.4f} | cH: {:.10f} | loss: {:.5f} | te: {} | tl: {} | lr: {}"
            print(print_string.format(self.epoch, batch_idx, samples_per_sec, losses["loss_HiS"].cpu().data, losses["loss_MiS"].cpu().data,
                                    losses["loss_consis_HiS"].cpu().data, losses["loss"].cpu().data,
                                    sec_to_hm_str(time_sofar), sec_to_hm_str(training_time_left), self.model_optimizer.state_dict()['param_groups'][0]['lr']))
