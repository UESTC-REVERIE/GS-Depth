...
class DepthDecoder_MSF_GS_FiT_Inter(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True, 
                 use_gs=False, gs_scale=0, min_depth=0.1, max_depth=100.0, height=192, width=640):
        super(DepthDecoder_MSF_GS_FiT_Inter, self).__init__()
    ...
    def forward(self, input_features, inv_K, K, frame_index):
        self.outputs = {}
        ...
        if self.use_gs:
            if self.gs_scale == 0:
                ...
                d0_3 = d0_3 + e3
                d0_4 = d0_4 + e4
                self.outputs[("gs_feature", 2)] = d0_0
                self.outputs[("gs_feature", 4)] = d0_1
                self.outputs[("gs_feature", 8)] = d0_2
                self.outputs[("gs_feature", 16)] = d0_3
                self.outputs[("gs_feature", 32)] = d0_4
            elif self.gs_scale == 2:
                d0_0 = d0_0 + e0
                self.outputs[("gs_feature", 2)] = d0_0
            elif self.gs_scale == 4:
