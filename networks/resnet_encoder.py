...
def resnet_multiimage_input(num_layers, pretrained=False, num_input_images=1):
    ...
    if pretrained:
        # 动态获取对应的权重类别
        weights_class = getattr(models, f'ResNet{num_layers}_Weights')
        # 加载预训练模型
        resnet_model = getattr(models, f'resnet{num_layers}')(weights=weights_class.DEFAULT)
...
class ResnetEncoder(nn.Module):
    ...
    def __init__(self, num_layers, pretrained, num_input_images=1):
        ...
        if num_layers > 34:
            self.num_ch_enc[1:] = self.num_ch_enc[1:] * 4       #[64, 256, 512, 1024, 2048]
