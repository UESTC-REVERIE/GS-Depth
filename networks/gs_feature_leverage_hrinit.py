...
class GaussianFeatureLeverage(nn.Module):
    ...
    def __init__(self, num_ch_in, scales, height=192, width=640, leveraged_feat_ch=64, min_depth=0.1, max_depth=100.0, num_ch_concat=0, gs_scale=0, split_dimensions = [3,1,3,4], gs_num_pixel=2):
        ...
        for i in range(gs_num_pixel):
            ...
            self.convs[("gs_feature_leve_conv", i)] = nn.Sequential(
                ConvBlock(total_ch_in, total_ch_in // 2),
                ConvBlock(total_ch_in // 2, leveraged_feat_ch)
            )
        # 反投影得到高斯中心
        ...
        for i in range(4):
            ...
            # _num_ch_in = leveraged_feat_ch * self.gs_num_pixel
            # _num_ch_in = leveraged_feat_ch * self.gs_num_pixel + 3
            _num_ch_in = leveraged_feat_ch * self.gs_num_pixel + self.num_ch_in[i]
    ...
    def forward(self, init_features, colors, init_disps, inv_K, K):
        ...
        # init_disp融合
        for i in range(4):
            features = features + [init_disps[self.gs_scale]]
        # color融合
        ...
        for i in range(4): # different scales feature
            ...
            # TODO 修改init_decoder使用hrnet结构, 抛弃 multi-scale loss
            # fused_features = gs_features # 融合其他特征
            # fused_features = gs_features + [colors[i+2]] # 融合其他特征
            fused_features = gs_features + [init_features[i]]
