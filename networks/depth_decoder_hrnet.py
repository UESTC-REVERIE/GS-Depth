...
class DepthDecoder_MSF_GS_FiTAlter_DST(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True, 
        ...
        if self.use_gs:
            ...
            self.convs[("gs_rotation_conv"), 0, 16] = ConvBlock(self.num_ch_enc[4], self.num_ch_enc[4])
            self.convs[("gs_rotation_conv"), 1, 16] = Conv3x3(self.num_ch_enc[4], 4)
            self.convs[("gs_rotation_conv"), 0, 8] = ConvBlock(self.num_ch_enc[3], self.num_ch_enc[3])
            self.convs[("gs_rotation_conv"), 1, 8] = Conv3x3(self.num_ch_enc[3], 4)
            self.convs[("gs_rotation_conv"), 0, 4] = ConvBlock(self.num_ch_enc[2], self.num_ch_enc[2])
            self.convs[("gs_rotation_conv"), 1, 4] = Conv3x3(self.num_ch_enc[2], 4)
            # 3. scale head
            ...
            self.convs[("gs_scale_conv"), 0, 16] = ConvBlock(self.num_ch_enc[4], self.num_ch_enc[4])
            self.convs[("gs_scale_conv"), 1, 16] = Conv3x3(self.num_ch_enc[4], 3)
            self.convs[("gs_scale_conv"), 0, 8] = ConvBlock(self.num_ch_enc[3], self.num_ch_enc[3])
            self.convs[("gs_scale_conv"), 1, 8] = Conv3x3(self.num_ch_enc[3], 3)
            self.convs[("gs_scale_conv"), 0, 4] = ConvBlock(self.num_ch_enc[2], self.num_ch_enc[2])
            self.convs[("gs_scale_conv"), 1, 4] = Conv3x3(self.num_ch_enc[2], 3)
            # 4. opacity head
            ...
            self.convs[("gs_opacity_conv"), 0, 16] = ConvBlock(self.num_ch_enc[4], self.num_ch_enc[4])
            self.convs[("gs_opacity_conv"), 1, 16] = Conv3x3(self.num_ch_enc[4], 1)
            self.convs[("gs_opacity_conv"), 0, 8] = ConvBlock(self.num_ch_enc[3], self.num_ch_enc[3])
            self.convs[("gs_opacity_conv"), 1, 8] = Conv3x3(self.num_ch_enc[3], 1)
            self.convs[("gs_opacity_conv"), 0, 4] = ConvBlock(self.num_ch_enc[2], self.num_ch_enc[2])
            self.convs[("gs_opacity_conv"), 1, 4] = Conv3x3(self.num_ch_enc[2], 1)
            # 5. feature head
            ...
            self.convs[("gs_feature_conv"), 0, 16] = ConvBlock(self.num_ch_enc[4], self.num_ch_enc[4])
            self.convs[("gs_feature_conv"), 1, 16] = ConvBlock(self.num_ch_enc[4], 64)
            self.convs[("gs_feature_conv"), 2, 8] = ConvBlock(64, self.num_ch_enc[2])
            
            self.convs[("gs_feature_conv"), 0, 8] = ConvBlock(self.num_ch_enc[3], self.num_ch_enc[3])
            self.convs[("gs_feature_conv"), 1, 8] = ConvBlock(self.num_ch_enc[3], 64)
            self.convs[("gs_feature_conv"), 2, 4] = ConvBlock(64, self.num_ch_enc[1])
            
            self.convs[("gs_feature_conv"), 0, 4] = ConvBlock(self.num_ch_enc[2], self.num_ch_enc[2])
            self.convs[("gs_feature_conv"), 1, 4] = ConvBlock(self.num_ch_enc[2], 64)
            self.convs[("gs_feature_conv"), 2, 2] = ConvBlock(64, self.num_ch_enc[0])

            # 6. 3D GS Rasterizer
            self.feature_rasterizer_16 = Rasterize_Gaussian_Feature_FiT3D_v1(image_height=self.height//16, image_width=self.width//16)
            self.feature_rasterizer_8 = Rasterize_Gaussian_Feature_FiT3D_v1(image_height=self.height//8, image_width=self.width//8)
            self.feature_rasterizer_4 = Rasterize_Gaussian_Feature_FiT3D_v1(image_height=self.height//4, image_width=self.width//4)
            self.feature_rasterizer_2 = Rasterize_Gaussian_Feature_FiT3D_v1(image_height=self.height//2, image_width=self.width//2)
    ...
    def forward(self, input_features, inv_K, K):
        ...
        if self.use_gs and self.gs_scale == 0:
            ...
            bs = gs_input_features[32].shape[0]
            ...
            e4_position = e4_position.view(bs, -1, 3)
            # 2. rotation head, [B, H*W, 4]
            e4_rotation = self.convs[("gs_rotation_conv"), 0, 32](gs_input_features[32])
            ...
            e4_scale = self.convs[("gs_scale_conv"), 0, 32](gs_input_features[32])
            ...
            e4_opacity = self.convs[("gs_opacity_conv"), 0, 32](gs_input_features[32])
            ...
            e4_feature = self.convs[("gs_feature_conv"), 0, 32](gs_input_features[32])
            ...
            bs = gs_input_features[16].shape[0]
            # 1. position head [B, H*W, 3]
            ...
            e3_position = e3_position_16.permute(0, 2, 1).contiguous()
            e3_position = e3_position.view(bs, -1, 3)
            # 2. rotation head, [B, H*W, 4]
            e3_rotation = self.convs[("gs_rotation_conv"), 0, 16](gs_input_features[16])
            e3_rotation = F.normalize(self.convs[("gs_rotation_conv"), 1, 16](e3_rotation), dim=1, p=2)
            e3_rotation = e3_rotation.permute(0, 2, 3, 1).contiguous()
            e3_rotation = e3_rotation.view(bs, -1, 4)
            # 3. scale head, [B, H*W, 3]
            e3_scale = self.convs[("gs_scale_conv"), 0, 16](gs_input_features[16])
            e3_scale = torch.abs(self.convs[("gs_scale_conv"), 1, 16](e3_scale))
            e3_scale = e3_scale.permute(0, 2, 3, 1).contiguous()
            e3_scale = e3_scale.view(bs, -1, 3)
            # 4. opacity head, [B, H*W, 1]
            e3_opacity = self.convs[("gs_opacity_conv"), 0, 16](gs_input_features[16])
            e3_opacity = self.sigmoid(self.convs[("gs_opacity_conv"), 1, 16](e3_opacity))
            e3_opacity = e3_opacity.permute(0, 2, 3, 1).contiguous()
            e3_opacity = e3_opacity.view(bs, -1, 1)
            # 5. feature head, [B, H*W, C]
            e3_feature = self.convs[("gs_feature_conv"), 0, 16](gs_input_features[16])
            e3_feature = self.convs[("gs_feature_conv"), 1, 16](e3_feature)
            e3_feature = e3_feature.permute(0, 2, 3, 1).contiguous()
            e3_feature = e3_feature.view(bs, -1, e3_feature.shape[-1])
            ...
            bs = gs_input_features[8].shape[0]
            # 1. position head [B, H*W, 3]
            ...
            # 2. rotation head, [B, H*W, 4]
            e2_rotation = self.convs[("gs_rotation_conv"), 0, 8](gs_input_features[8])
            ...
            # 3. scale head, [B, H*W, 3]
            e2_scale = self.convs[("gs_scale_conv"), 0, 8](gs_input_features[8])
            ...
            # 4. opacity head, [B, H*W, 1]
            e2_opacity = self.convs[("gs_opacity_conv"), 0, 8](gs_input_features[8])
            ...
            # 5. feature head, [B, H*W, C]
            e2_feature = self.convs[("gs_feature_conv"), 0, 8](gs_input_features[8])
            ...
            bs = gs_input_features[4].shape[0]
            # 1. position head [B, H*W, 3]
            ...
            # 2. rotation head, [B, H*W, 4]
            e1_rotation = self.convs[("gs_rotation_conv"), 0, 4](gs_input_features[4])
            ...
            # 3. scale head, [B, H*W, 3]
            e1_scale = self.convs[("gs_scale_conv"), 0, 4](gs_input_features[4])
            ...
            # 4. opacity head, [B, H*W, 1]
            e1_opacity = self.convs[("gs_opacity_conv"), 0, 4](gs_input_features[4])
            ...
            # 5. feature head, [B, H*W, C]
            e1_feature = self.convs[("gs_feature_conv"), 0, 4](gs_input_features[4])
