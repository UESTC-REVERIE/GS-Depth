...
class DepthDecoder_MSF_GS_FiT_TBT(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True, 
                 use_gs=False, gs_scale=0, min_depth=0.1, max_depth=100.0, height=192, width=640):
        super(DepthDecoder_MSF_GS_FiT_TBT, self).__init__()
    ...
    def forward(self, input_features, inv_K, K):
        ...
        e0 = self.convs[("parallel_conv"), 0, 0](e0)
        e1 = self.convs[("parallel_conv"), 0, 1](e1)
        e2 = self.convs[("parallel_conv"), 0, 2](e2)
        e3 = self.convs[("parallel_conv"), 0, 3](e3)
        e4 = self.convs[("parallel_conv"), 0, 4](e4)
