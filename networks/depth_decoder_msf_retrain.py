    ...
    def forward(self, input_features, inv_K, K):
        ...
        if self.use_gs:
            if self.gs_scale == 0:
                ...
                # e4_position = e4_position_32.reshape(bs, -1, 3)
                # 2. rotation head, [B, H*W, 4]
                e4_rotation = self.convs[("gs_rotation_conv"), 0, 32](e4)
                # e4_rotation = torch.abs(F.normalize(self.convs[("gs_rotation_conv"), 1, 32](e4_rotation), dim=1, p=2))
                ...
                # e4_rotation = e4_rotation.reshape(bs, -1, 4)
                # 3. scale head, [B, H*W, 3]
                e4_scale = self.convs[("gs_scale_conv"), 0, 32](e4)
                # e4_scale = torch.abs(F.normalize(self.convs[("gs_scale_conv"), 1, 32](e4_scale), dim=1, p=2))
                ...
                # e4_scale = e4_scale.reshape(bs, -1, 3)
                ...
                # e4_opacity = e4_opacity.reshape(bs, -1, 1)
                ...
                # e4_feature = e4_feature.reshape(bs, -1, e4_feature.shape[1])
                gs_feature = self.feature_rasterizer_32(e4_position, e4_rotation, e4_scale, e4_opacity, e4_feature, inv_K[4], K[4])
                # e4 = gs_feature.view(e4.shape[0], e4.shape[2], e4.shape[3], -1)
                # e4 = e4.transpose(0, 3, 1, 2)
                ...
                # e3_rotation = torch.abs(F.normalize(self.convs[("gs_rotation_conv"), 1, 16](e3_rotation), dim=1, p=2))
                ...
                # e3_scale = torch.abs(F.normalize(self.convs[("gs_scale_conv"), 1, 16](e3_scale), dim=1, p=2))
                ...
                # e3 = gs_feature.view(e3.shape[0], e3.shape[2], e3.shape[3], -1)
                # e3 = e3.transpose(0, 3, 1, 2)
                ...
                # e2_position = e2_position_8.reshape(bs, -1, 3)
                # 2. rotation head, [B, H*W, 4]
                e2_rotation = self.convs[("gs_rotation_conv"), 0, 8](e2)
                # e2_rotation = torch.abs(F.normalize(self.convs[("gs_rotation_conv"), 1, 8](e2_rotation), dim=1, p=2))
                ...
                # e2_rotation = e2_rotation.reshape(bs, -1, 4)
                # 3. scale head, [B, H*W, 3]
                e2_scale = self.convs[("gs_scale_conv"), 0, 8](e2)
                # e2_scale = torch.abs(F.normalize(self.convs[("gs_scale_conv"), 1, 8](e2_scale), dim=1, p=2))
                ...
                # e2_scale = e2_scale.reshape(bs, -1, 3)
                # 4. opacity head, [B, H*W, 1]
                ...
                # e2_opacity = e2_opacity.reshape(bs, -1, 1)
                # 5. feature head, [B, H*W, C]
                ...
                # e2_feature = e2_feature.reshape(bs, -1, e2_feature.shape[1])
                gs_feature = self.feature_rasterizer_8(e2_position, e2_rotation, e2_scale, e2_opacity, e2_feature, inv_K[2], K[2])
                # e2 = gs_feature.view(e2.shape[0], e2.shape[2], e2.shape[3], -1)
                # e2 = e2.transpose(0, 3, 1, 2)
                ...
                # e1_position = e1_position_4.reshape(bs, -1, 3)
                # 2. rotation head, [B, H*W, 4]
                e1_rotation = self.convs[("gs_rotation_conv"), 0, 4](e1)
                # e1_rotation = torch.abs(F.normalize(self.convs[("gs_rotation_conv"), 1, 4](e1_rotation), dim=1, p=2))
                ...
                # e1_rotation = e1_rotation.reshape(bs, -1, 4)
                # 3. scale head, [B, H*W, 3]
                e1_scale = self.convs[("gs_scale_conv"), 0, 4](e1)
                # e1_scale = torch.abs(F.normalize(self.convs[("gs_scale_conv"), 1, 4](e1_scale), dim=1, p=2))
                ...
                # e1_scale = e1_scale.reshape(bs, -1, 3)
                # 4. opacity head, [B, H*W, 1]
                ...
                # e1_opacity = e1_opacity.reshape(bs, -1, 1)
                # 5. feature head, [B, H*W, C]
                ...
                # e1_feature = e1_feature.reshape(bs, -1, e1_feature.shape[1])
                gs_feature = self.feature_rasterizer_4(e1_position, e1_rotation, e1_scale, e1_opacity, e1_feature, inv_K[1], K[1])
                # e1 = gs_feature.view(e1.shape[0], e1.shape[2], e1.shape[3], -1)
                # e1 = e1.transpose(0, 3, 1, 2)
                ...
                # e0_position = e0_position_2.reshape(bs, -1, 3)
                # 2. rotation head, [B, H*W, 2]
                e0_rotation = self.convs[("gs_rotation_conv"), 0, 2](e0)
                # e0_rotation = torch.abs(F.normalize(self.convs[("gs_rotation_conv"), 1, 2](e0_rotation), dim=1, p=2))
                ...
                # e0_rotation = e0_rotation.reshape(bs, -1, 4)
                # 3. scale head, [B, H*W, 3]
                e0_scale = self.convs[("gs_scale_conv"), 0, 2](e0)
                # e0_scale = torch.abs(F.normalize(self.convs[("gs_scale_conv"), 1, 2](e0_scale), dim=1, p=2))
                ...
                # e0_scale = e0_scale.reshape(bs, -1, 3)
                ...
                # e0_opacity = e0_opacity.reshape(bs, -1, 1)
                # 5. feature head, [B, H*W, C]
                ...
                # e0_feature = e0_feature.reshape(bs, -1, e0_feature.shape[1])
                gs_feature = self.feature_rasterizer_2(e0_position, e0_rotation, e0_scale, e0_opacity, e0_feature, inv_K[0], K[0])
                # e0 = gs_feature.view(e0.shape[0], e0.shape[2], e0.shape[3], -1)
                # e0 = e0.transpose(0, 3, 1, 2)
