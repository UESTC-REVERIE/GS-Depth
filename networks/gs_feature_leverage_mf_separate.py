...
class GaussianFeatureLeverage(nn.Module):
    ...
    def __init__(self, num_ch_in, scales, height=192, width=640, leveraged_feat_ch=64, min_depth=0.1, max_depth=100.0, num_ch_concat=0, gs_scale=0, split_dimensions = [3,1,3,4], gs_num_pixel=2):
        ...
        # 融合lookup_features
        total_ch_in = list(range(4))
        for i  in range(4):
            if i == 0:
                total_ch_in[i] = self.num_ch_in[i] * 2 + self.num_ch_concat + self.num_depth_bins
            else:
                total_ch_in[i] = self.num_ch_in[i] * 2 + self.num_ch_concat
        
        # print(f"using option: predict {self.gs_num_pixel} gs per pixel")
        # rotation scale opacity depth-offset
        for i in range(4):
            self.convs[("gs_head_conv", i)] = self.create_gaussian_head(total_ch_in[i], sum(self.split_dimensions))
            self.convs[("gs_feature_leve_conv", i)] = nn.Sequential(
                ConvBlock(total_ch_in[i], total_ch_in[i]),
                ConvBlock(total_ch_in[i], leveraged_feat_ch)
            )
            # 反投影得到高斯中心
            self.backprojector[i] = BackprojectDepth_PointCloud(
                height = self.height // (2 ** (i + 2)),
                width = self.width // (2 ** (i + 2))
            )
    ...
    def forward(self, init_features, colors, init_disps, inv_K, K, T=None, lookup_features=None, lookup_T=None):
        ...
        bs = init_disps[0].shape[0]
        depth = list(range(4))
        for i in range(4):
            _ , depth[i] = disp_to_depth(init_disps[i], self.min_depth, self.max_depth)
            depth[i] = depth[i].clamp(self.min_depth, self.max_depth)
        
        input_features = OrderedDict()
        
        for i in range(4):
            features = []
            features = features + [init_features[i]] # init features
            features = features + [lookup_features[i]] # lookup features
            
            features = features + [init_disps[i]] # init disp
            features = features + [colors[i+2]] # color
        ...
        if lookup_features is not None: # 有可用的multi-frames
            ...
            # cost volume 融合
            input_features[0] = input_features[0] +  [cost_volume]
            
        for i in range(4):
            input_features[i] = torch.cat(input_features[i], 1) 
            
        # print("fused feature shape is ", feature.shape)
        position, rotation, scale, opacity, gs_feature = (
            list(range(4)) for _ in range(5)
        )
        
        
        for i in range(4):
            
            gs_params = self.convs[("gs_head_conv", i)](input_features[i])
            position[i], rotation[i], scale[i], opacity[i] = self.gs_generator(depth[i],bs,inv_K[i+2],gs_params, self.backprojector[i])
            
            gs_feature[i] = self.convs[("gs_feature_leve_conv", i)](input_features[i])
        ...
        for i in range(4): # different scales feature
            fused_features = []
            fused_features = fused_features + [self.rasterizer[i](position[i], rotation[i], scale[i], opacity[i], gs_feature[i], K[i+2])]

            if i == 0:
                fused_features = fused_features + [init_features[i]] + [colors[i+2]] + [init_disps[i]] + [cost_volume] # 加入cost volume
            else:    
                fused_features = fused_features + [init_features[i]] + [colors[i+2]] + [init_disps[i]] # 融合其他特征
        ...
        return leveraged_features, None
    
    def gs_generator(self, depth, bs, inv_k, gs_params, backprojector):
        
        offset_s, opacity_s, scale_s, rotation_s = gs_params.split(self.split_dimensions, dim=1)

        # 1. Position calculation
        _position = backprojector(depth, inv_k)
