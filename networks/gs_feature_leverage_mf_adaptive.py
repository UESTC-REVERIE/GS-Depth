...
class GaussianFeatureLeverage(nn.Module):
    ...
    def forward(self, init_features, colors, init_disps, inv_K, K, T=None, lookup_features=None, lookup_T=None):
        ...
        with torch.no_grad():
            running_min_depth = depth.min().item()
            running_max_depth = depth.max().item()
            # 深度范围平滑处理
            running_max_depth = 0.9 * running_max_depth + 0.1 * self.max_depth
            running_min_depth = 0.9 * running_min_depth + 0.1 * self.min_depth
            self.compute_depth_bins(running_min_depth, running_max_depth)
