...
class HRDepthDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True):
        ...
        self.convs[("parallel_conv", 4, 0)] = nn.Sequential(
            ConvBlock(stage_4_ch_num, stage_4_ch_num),
            ConvBlock(stage_4_ch_num, stage_4_ch_num)
        )
        stage_3_ch_num = self.num_ch_dec[3]
        self.convs[("parallel_conv", 3, 0)] = nn.Sequential(
            ConvBlock(stage_3_ch_num, stage_3_ch_num),
            ConvBlock(stage_3_ch_num, stage_3_ch_num)
        )
        self.convs[("parallel_conv", 3, 1)] = nn.Sequential(
            ConvBlock(stage_3_ch_num, stage_3_ch_num),
            ConvBlock(stage_3_ch_num, stage_3_ch_num)
        )
        stage_2_ch_num = self.num_ch_dec[2]
        self.convs[("parallel_conv", 2, 0)] = nn.Sequential(
            ConvBlock(stage_2_ch_num, stage_2_ch_num),
            ConvBlock(stage_2_ch_num, stage_2_ch_num)
        )
        self.convs[("parallel_conv", 2, 1)] = nn.Sequential(
            ConvBlock(stage_2_ch_num, stage_2_ch_num),
            ConvBlock(stage_2_ch_num, stage_2_ch_num)
        )
        self.convs[("parallel_conv", 2, 2)] = nn.Sequential(
            ConvBlock(stage_2_ch_num, stage_2_ch_num),
            ConvBlock(stage_2_ch_num, stage_2_ch_num)
        )
