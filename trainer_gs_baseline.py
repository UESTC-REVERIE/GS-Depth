...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.pretrained_model_path is not None:
            self.load_pretrained_model(self.opt.pretraind_model_path, self.opt.pretrained_models_to_load)
        # endregion
    ...
    def load_pretrained_model(self, path, models_to_load):
        """Load model(s) from disk
        """
        path = os.path.expanduser(path)

        assert os.path.isdir(path), \
            "--pretrained_model_path should be a model file, but {} is a dir".format(path)
