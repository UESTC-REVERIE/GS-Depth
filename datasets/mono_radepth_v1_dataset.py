...
class MonoDataset(data.Dataset):
    ...
    def preprocess_HiS(self, inputs, color_aug, height_re_HiS, width_re_HiS, dx_HiS, dy_HiS, do_crop_aug):
        """Resize colour images to the required scales and augment if required
    ...
    def preprocess_LoS(self, inputs, color_aug, height_re_LoS, width_re_LoS, do_crop_aug):
        """Resize colour images to the required scales and augment if required
        ...
        for k in list(inputs):
            ...
            if "color" in k:
                ...
                for i in range(self.num_scales):
                    inputs[(n + "_MiS", im, i)] = self.resize_MiS(inputs[(n, im, i - 1)])
                    inputs[(n + "_HiS", im, i)] = self.resize_LoS(inputs[(n, im, i - 1)])
    ...
    def __getitem__(self, index):
        ...
        self.do_HiS = self.is_train and random.random() > 0.5
        inputs[("do_HiS")] = self.do_HiS 
        ...
        if self.do_HiS:
            ...
            inputs[("dxy_HiS")] = torch.Tensor((dx_LoS, dy_LoS))
            inputs[("resize_HiS")] = torch.Tensor((width_re_LoS, height_re_LoS))
        ...
        for scale in range(self.num_scales):
            ...
            if self.do_HiS:
                ...
                inputs[("K_HiS", scale)] = torch.from_numpy(K_LoS)
                inputs[("inv_K_HiS", scale)] = torch.from_numpy(inv_K_LoS)

        if self.gs_scale == 0:
            for scale in range(1, 6):
                K_gs_MiS = self.K.copy()
                K_gs_MiS[0, :] *= self.width // (2 ** scale)
                K_gs_MiS[1, :] *= self.height // (2 ** scale)
                inv_K_gs_MiS = np.linalg.pinv(K_gs_MiS)
                inputs[("K_gs_MiS", scale)] = torch.from_numpy(K_gs_MiS)
                inputs[("inv_K_gs_MiS", scale)] = torch.from_numpy(inv_K_gs_MiS)
                if self.do_HiS:
                    K_gs_HiS = self.K.copy()
                    K_gs_HiS[0, :] *= width_re_HiS // (2 ** scale)
                    K_gs_HiS[1, :] *= height_re_HiS // (2 ** scale)
                    inv_K_gs_HiS = np.linalg.pinv(K_gs_HiS)
                    inputs[("K_gs_HiS", scale)] = torch.from_numpy(K_gs_HiS)
                    inputs[("inv_K_gs_HiS", scale)] = torch.from_numpy(inv_K_gs_HiS)

                else:
                    K_gs_LoS = self.K.copy()                  
                    K_gs_LoS[0, :] *= width_re_LoS // (2 ** scale)
                    K_gs_LoS[1, :] *= height_re_LoS // (2 ** scale)
                    inv_K_gs_LoS = np.linalg.pinv(K_gs_LoS)
                    inputs[("K_gs_HiS", scale)] = torch.from_numpy(K_gs_LoS)
                    inputs[("inv_K_gs_HiS", scale)] = torch.from_numpy(inv_K_gs_LoS)
                
        else:
            scale = self.gs_scale
            K_gs_MiS = self.K.copy()
            K_gs_MiS[0, :] *= self.width // (scale)
            K_gs_MiS[1, :] *= self.height // (scale)
            inv_K_gs_MiS = np.linalg.pinv(K_gs_MiS)
            inputs[("K_gs_MiS", scale)] = torch.from_numpy(K_gs_MiS)
            inputs[("inv_K_gs_MiS", scale)] = torch.from_numpy(inv_K_gs_MiS)
            if self.do_HiS:
                K_gs_HiS = self.K.copy()
                K_gs_HiS[0, :] *= width_re_HiS // (scale)
                K_gs_HiS[1, :] *= height_re_HiS // (scale)
                inv_K_gs_HiS = np.linalg.pinv(K_gs_HiS)
                inputs[("K_gs_HiS", scale)] = torch.from_numpy(K_gs_HiS)
                inputs[("inv_K_gs_HiS", scale)] = torch.from_numpy(inv_K_gs_HiS)
                ...
                K_gs_LoS = self.K.copy()
                K_gs_LoS[0, :] *= width_re_LoS // (scale)
                K_gs_LoS[1, :] *= height_re_LoS // (scale)
                inv_K_gs_LoS = np.linalg.pinv(K_gs_LoS)
                inputs[("K_gs_HiS", scale)] = torch.from_numpy(K_gs_LoS)
                inputs[("inv_K_gs_HiS", scale)] = torch.from_numpy(inv_K_gs_LoS)
        ...
        if self.do_HiS:
            self.preprocess_HiS(inputs, color_aug, height_re_HiS, width_re_HiS, dx_HiS, dy_HiS, do_crop_aug)
        else:
            self.preprocess_LoS(inputs, color_aug, height_re_LoS, width_re_LoS, do_crop_aug)
